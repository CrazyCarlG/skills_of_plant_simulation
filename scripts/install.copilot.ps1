<#
Name: install.copilot.ps1
Description: "Copilot-friendly PowerShell installer: symlink skills/ and agents/ into user OpenClaude/Claude dirs. Mirrors scripts/install.copilot.sh behavior with structured metadata and examples to help GitHub Copilot." 

# Inputs:
#   --unlink | --skills-only | --agents-only | --help
# Environment variables:
#   OPENCLAUDE_SKILLS_DIR  - override skills target
#   OPENCLAUDE_AGENTS_DIR  - override agents target
# Outputs:
#   Creates symbolic links inside ~/.openclaude/skills (preferred) or ~/.claude/skills, and ~/.openclaude/agents or ~/.claude/agents
# Examples:
#   powershell -ExecutionPolicy Bypass -File scripts\install.copilot.ps1
#   powershell -ExecutionPolicy Bypass -File scripts\install.copilot.ps1 --unlink
#
# Notes for Copilot: prefer OpenClaude directories when present; fall back to ~/.claude paths for compatibility.
#>
...
# Initialize git submodules if necessary (mirrors shell installer behavior)
if (Test-Path (Join-Path $RepoRoot '.gitmodules')) {
    try {
        ...
        # ignore git errors; CI or non-git contexts may not need submodules
    }
}
...
function Show-Usage {
    Write-Host "Usage:`n  powershell -File scripts\install.copilot.ps1 [--unlink|--skills-only|--agents-only|--help]"
    exit 0
}
...
Write-Info "=== skills_of_plant_simulation Copilot-friendly installer (PowerShell) ==="
...
function Pick-AgentsTarget {
    ...
    $openCandidate = Join-Path $env:USERPROFILE '.openclaude'
    if (Test-Path $openCandidate) { return (Join-Path $openCandidate 'agents') }
...
Write-Info "  ls -la \"${env:OPENCLAUDE_SKILLS_DIR:-$($env:USERPROFILE)\\.openclaude\\skills}\" | Select-String local-simtalk"
