...
def main():
    ...
    if args.restore:
        with open(args.backup, "r", encoding="utf-8") as f:
            original = f.read()
        # Use chr(10) joins so multi-line programs survive the trip.
        lines = original.split("\n")
        rhs = " + chr(10) + ".join(quote(line) for line in lines)
        code = (
    ...
    #    v15+ readlog is degraded — many runs return `execute success`
    #    without capturing `print(...)`. So we DON'T trust readlog to
    #    tell us whether the object is a Method. Instead, we attempt
    #    the typecheck and rely on Quirk #7 (Quirk #7 = `result:success`
    #    + `log:"code execute failed..."`) to surface a void path. If
    #    the typecheck exits 0 (success), we proceed; Quirk #7 (exit 11)
    #    means the path didn't resolve to a real object.
    typecheck_code = (
        'var obj: object; obj := str_to_obj("' + args.path + '"); '
        'print "###MARKER###"; '
        'if obj = void then '
    ...
    if rc == 11:
        # Quirk #7 — str_to_obj returned void or internalclasstype lookup failed
        print("[typecheck] FAILED (Quirk #7 soft failure). "
              "Path likely does not resolve. Path: " + args.path,
              file=sys.stderr)
        print(out)
        sys.exit(11)
    if rc != 0:
    ...
    if rc != 0:
        ...
        sys.exit(11)
    rc2, log2 = readlog()
    # readlog may return rc=0 or rc=20 (v15+ degraded warning). Both
    # carry usable stdout. Only fall back to `out` if readlog failed.
    log_text = log2 if rc2 in (0, 20) else out
