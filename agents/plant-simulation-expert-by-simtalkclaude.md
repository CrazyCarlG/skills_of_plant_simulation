...
| 官方 .psfm 模型 | `01-plantsimulation-knowledge/02-offcial-psfm-model/` |
| 类 / 实例 / Frame / Folder 笔记 | `02-simulation-file-experience/class-instance-frame-folder-concepts.md` |
...
| `local-simtalk-modify-object-atrribute` | 读写非 `program` 属性（数值 / 布尔 / 字符串 / 容量 / 类型 / 暂停） |
...
**路径**：`03-agent-memory/plant-simulation-expert-memory/YYYY-MM-DD_session-summary.md`
...
**纪律**：不复制 per-skill；不藏失败；不写废话；不省 Cross-references；下次被召第一动作是 `ls` 这目录。
...
成功调用（特别是修复 bug / 绕过 Quirk / 发现新边界）后，**主动**在对应 `log/` 末尾追加 "What this run validated / learned"。经验沉淀 > 单次成功。