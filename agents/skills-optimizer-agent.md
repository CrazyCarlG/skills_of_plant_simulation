...
1. **[quirk-001]** log shows `FlowControl.EntryBlocking` rejects `true`/`false`
   ...
   - **Evidence:** `skills/local-simtalk-modify-object-atrribute/log/2026-08-26_*` §Findings 1
...
| 2026-08-27 | `local-simtalk-modify-object-atrribute` | 0 | 3 | 1 | verify P1 against current script |
...
3. **不引用未读过的 log**——任何 evidence 行都必须来自本次任务真实读过的文件。
4. **不混淆"log 描述"与"agent 判断"**——报告里用 `> Evidence:` 引用原文，用 `Analysis:` 写 agent 自己的归纳。
