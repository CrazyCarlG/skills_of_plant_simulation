...
## 附:INDEX.md 行追加(per CONTRIBUTING §6.1 rule 2)

在 `02-simulation-file-experience/03-workflow-playbook/INDEX.md §经验 Log per-entry files` 表格末尾追加:
...
## 附:`skill-call-playbook.md §经验 Log` 末尾 pointer(per CONTRIBUTING §6.1 rule 3)

在 `02-simulation-file-experience/03-workflow-playbook/skill-call-playbook.md §经验 Log` 末尾追加:
...
## 附:frontmatter bump(per CONTRIBUTING §6.1 rule 4)

**`skill-call-playbook.md`** frontmatter 改:
```yaml
---
...
**`03-workflow-playbook/INDEX.md`** frontmatter 改:
```yaml
---
last_updated: 2026-09-01
contributors: [@z004bjuu, @plant-simulation-expert, @plant-simulation-experience-curator]
...
**`CONTRIBUTING.md`** §6 文字无需改(per-entry file 协议本身不变,只是新增一个文件)。