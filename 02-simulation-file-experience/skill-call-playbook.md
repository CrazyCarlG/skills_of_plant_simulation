...
>
> 与既有两份文档互补：
> - `class-instance-frame-folder-concepts.md` — 讲 **Plant Simulation 领域概念**（Class/Instance/Frame/Folder）
> - `simtalkclaude-best-practices.md` — 讲 **.SimtalkClaude bridge 内部**（协议、scratch buffer、ErrorHandler）
...
| `read-library` | BATCH=8 在小方法上够用；对内嵌多行字符串的方法会丢内容 | 单方法一次 + sleep(1.2) + readlog；或 marker 模式 `###PRG_START###` / `###PRG_END###` 抓 print 输出 | `simtalkclaude-best-practices.md` §5.2 + `lifelines.md` §5 |
...
完整 Origin/Class/OriginRoot 矩阵见 `class-instance-frame-folder-concepts.md` §2.3。
...
| `local-simtalk-modify-object-atrribute` | `log/01_*.log` ~ `log/80_*.log`（80 个细分 session）+ `log/SUMMARY.md` | SUMMARY.md Round 1+2（MaterialFlow 全覆盖 + Resources/InformationFlow） |
