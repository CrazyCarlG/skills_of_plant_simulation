...
scope: Factory51 + SimtalkClaude 集成的总入口（目录导览 + 子文件索引）
---

# factory51 — Factory51 + SimtalkClaude 集成经验

> 在 Siemens 官方 **Factory51** 示例模型（`02-official-psfm-model/Factory51/Factory51.psfm`）
> 之上导入 **SimtalkClaude**（TCP 远程驱动桥）后的整理与经验沉淀。
> 目录名 `factory51` 沿用用户拼写，**故意不更名**，避免破坏已存在的引用。
...
2. **SimtalkClaude v2 比 v1（已记录在 `../../02-bridge-tool/simtalkclaude-v1-and-v2.md`）多了三个真正有价值的模式**：
...
- `../../02-bridge-tool/simtalkclaude-v1-and-v2.md` —— SimtalkClaude v1+v2 的完整经验
- `../../02-bridge-tool/simtalkclaude-v1-v2-delta.md` —— v1 vs v2 速查表
- `../../03-workflow-playbook/skill-call-playbook.md` —— 9 个 skill 的使用 playbook
- `../ctu-warehouse/p4-ctu-class-inheritance.md` —— 另一份"反向验证类定义存在性"的实例参考
- `01-plantsimulation-knowledge/02-official-psfm-model/Factory51/model-know-how/Factory51-模型结构.md` —— Factory51 模型的结构笔记（已有）
...
- `01-plantsimulation-knowledge/02-official-psfm-model/Factory51/` —— Factory51 模型 bundle
