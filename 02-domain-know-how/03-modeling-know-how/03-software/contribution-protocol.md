...
scope: 经验沉淀的贡献协议(append-only 时间线 + entry 字段格式 + Supersede 模式 + 自检清单)
---

# 经验沉淀贡献协议

本目录是**经验沉淀**,不是 Plant Simulation 知识库。经验沉淀的强约束是**多人增量写入**:每位工程师在跑任务时踩到坑 / 验证模式,都能在不打扰别人、不重写主体、不制造 git 冲突的前提下,把自己的发现追加进去。
...
按 README §经验沉淀协议 的触发点(session 末尾 / 里程碑 / 新坑 / 跨 session 重复):

1. **grep 现有文件**——`grep -r "<keyword>" 02-domain-know-how/` 确认是新坑
2. **选文件**——按"路径分配"5 条规则(skill bug → skills/<name>/log/;领域知识 → 03-;桥 → 02-;跨 skill 工作流 → 03-;模型特定 → 01- 或 04-)
3. **追加 entry**——在该文件**末尾的"## 经验 Log"区**追加一条;不要改主体
4. **frontmatter bump**——更新 `last_updated`,在 `contributors` 里加自己(首次)
5. **git commit**——commit message 建议 `<file>: +<tag1>, <tag2> by @user`
...
- 删 / 改老 entry 正文
- 不带 issue 引用就改主体区
- 粘贴整段 session 流水(session summary 归 `05-modeling-experience/`,不归本目录)
- 新建 "X 发现汇总.md" / "踩坑日记.md" / 任何游离在主题目录之外的 `.md`
- 把同一条 entry 同时写到多个文件(用 `see also` 字段交叉引用)

## 四、自检清单(沉淀前 30 秒)

- [ ] **新坑 vs 已有坑?** `grep -r "<keyword>" 02-domain-know-how/` —— 已存在就 update 现有 entry(加 supersede 标记),不新建。
- [ ] **文件选对了吗?** 走"路径分配"5 条规则。skill bug 不归本目录。
...
## 六、per-entry file 约定(2026-08-31 起强制)

`03-modeling-know-how/03-software/skill-orchestration-guide.md` 的 §经验 Log 从 2026-08-31 起**不再内嵌 entry 正文**——每条 entry 单独成 `.md` 文件。

**为什么**:playbook 的 entry 越来越长(最长达 23 行 + 1 个决策表),内嵌导致:
- 单条 entry 的 git diff 被其他 6 节主体变更噪声淹没
- tag-based 检索必须 grep 整个文件
- per-entry supersede 标记无法精准定位

### 6.1 强制规则

1. **新 entry 必须独立成文件**:`<YYYY-MM-DD> by @<author> — <entry-title>.md`
2. **必须在 INDEX §经验 Log per-entry files 表格加一行**(Date / Author / tags)
3. **必须在 `skill-orchestration-guide.md §经验 Log` 末尾追加 1 行 pointer**(标题保留供反向 grep,正文改为"→ 详见 [file]...")
4. **frontmatter 三处同时 bump**:`last_updated` + `contributors`(三个文件:playbook.md / INDEX.md / CONTRIBUTING.md 的 §6 提及)
...
本目录 `01-objects/` / `02-simtalk/` / `04-modeling-example/` / `05-modeling-experience/` 的 §经验 Log **目前不强制**走 per-entry file——保留内嵌正文形式。

## 历史

- 2026-08-28 创建本文件(@z004bjuu + @plant-simulation-expert)
- 2026-08-31 新增 §6 playbook per-entry file 约定(@plant-simulation-experience-curator)
- 2026-09-01 重组到 `02-domain-know-how/03-modeling-know-how/03-software/`,内容主体保留(@plant-simulation-expert)
</content>
</invoke>