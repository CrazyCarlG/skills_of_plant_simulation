...
scope: Plant Simulation 对象四个核心概念(Class / Instance / Frame / Folder)+ 可靠的判定方法(Origin/Class/OriginRoot 三元组)+ Frame 双重身份
---

# Plant Simulation 对象概念

写 / 读任何 Plant Simulation 对象前必读。理解 Class / Instance / Frame / Folder 是后续 `derive` / `duplicate` / `create` 等所有操作的前提。

## 一、四个核心对象概念

Plant Simulation 里一切"对象"按用途分四类:
...
| `NumChildren` 含义 | 类库子项数 | **结构**子节点数(**不含** 2D 视图里的 placed 实例) |
